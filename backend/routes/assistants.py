...
async def spawn_job(assistant_id: str, body: SpawnJob):
    ...
    try:
        ...
        await db.execute(
            """INSERT INTO jobs (id, title, prompt, model, priority, work_dir, project_id, permissions, assistant_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (job_id, body.title, merged_prompt, model, body.priority, work_dir, project_id, permissions_json, assistant_id),
        )
        await db.commit()
